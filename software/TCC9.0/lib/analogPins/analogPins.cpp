...
int Analog::Read_Buttons()
{
    ...
    if (getBttnRead() < 75)  return getbtnDOWN();      //0.25V
    if (getBttnRead() < 125)  return getbtnUP();   //0.50V
...
void Analog::trailSetup()
{
    ...
    int i = getStepCount();
    for ( setSensorRead(analogRead(1)); getSensorRead() < 900 && getSensorRead() > 590; setStepCount(i++)) 
    {
    ...
    for ( setSensorRead(analogRead(1)); getSensorRead() > 590; setStepCount(i--)) 
    {
        mStepper myStepper( getmpin1() , getmpin2() );
        myStepper.setSpeed(getVelPPM());
        myStepper.move(-1);
